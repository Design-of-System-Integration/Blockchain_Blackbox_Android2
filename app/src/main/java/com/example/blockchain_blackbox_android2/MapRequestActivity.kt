...
import com.example.blockchain_blackbox_android2.databinding.ActivityMapRequestBinding
...
class MapRequestActivity : AppCompatActivity() {
    private lateinit var binding: ActivityMapRequestBinding
    private lateinit var mapView: MapView
    private var markerTag = 0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityMapRequestBinding.inflate(layoutInflater)
        setContentView(binding.root)

        mapView = binding.mapView
        mapView.setDaumMapApiKey("bb1e558b2c0e995b462c35ff1e90fca1")
    ...
    fun main(mapSearchInput: String) = runBlocking {
        ...
        // 카카오 REST API 키
        val apiKey = "bb1e558b2c0e995b462c35ff1e90fca1"
    ...
    private fun showAddMarkerDialog(mapView: MapView, mapPoint: MapPoint) {
        val dialog = AlertDialog.Builder(this@MapRequestActivity)
            .setMessage("이 위치에 마커를 찍으시겠습니까?")
            .setPositiveButton("확인") { dialog, which ->
                val marker = MapPOIItem()
                marker.itemName = "마커" // 마커 이름
                marker.tag = markerTag // 마커 고유 태그 (원하는 값으로 설정)
                marker.mapPoint = mapPoint // 클릭한 위치에 마커 추가
                marker.markerType = MapPOIItem.MarkerType.RedPin // 마커 아이콘 타입
                marker.selectedMarkerType = MapPOIItem.MarkerType.BluePin // 마커 선택 시 아이콘 타입
                mapView.addPOIItem(marker)
                markerTag++

                mapView.invalidate() // 맵뷰 업데이트
            }
